...
def generar_ruido_rosa(duracion: float, fs: int) -> np.ndarray:
    ...
    Parameters
    ----------
    duracion : float --> Duracion de la senal en segundos.
    ...
    # Aplicamos la transformada de Fourier a través de la funcion np.fft.rfft().
    ...
    # Dividimos cada componente por sqrt(f), y omitimos f=0 para evitar división por cero.
