...
def generar_sine_sweep(f1: float = 20.00, f2: float = 20000.00, duracion: float = 2, fs: int = 48000) -> tuple[np.ndarray, np.ndarray]:
    ...
    Definiciones
    ------------
    El sine sweep logaritmico es la senal de excitacion preferida para la medicion 
    de respuestas al impulso segun la tecnica de Farina (2000).
    ...
    # Luego, dividimos por la frecuencia de muestreo para obtener tiempos reales en lugar de muestras, 
    # y armar el vector tiempo.

    muestras = duracion * fs
    t = np.arange(int(muestras)) / fs

    # Construimos la fase del sine sweep senoidal a partir de la tecnica de Farina.
    ...
    # Ahora, para construir el filtro inverso, primero invertimos el arreglo del sine sweep 
    # en el tiempo usando la funcion de slicing.

    sweep_invertido = sweep[::-1]

    # El sine sweep tiene mucha energía acumulada en los graves y poca en los agudos, por lo que queremos 
    # que el filtro inverso compense esta situacion. Para ello, creamos una envolvente que depende 
    # del vector tiempo, la cual atenúa las frecuencias bajas y amplifica las altas.
    ...
    # La función creada devuelve tanto el sine sweep logarítmico como su filtro inverso. 
    # Ambos valores son arreglos.
